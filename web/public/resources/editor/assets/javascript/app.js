...
$(document).ready(function () {
    $("#button-menu-hide").click()
    var urlParams = new URLSearchParams(window.location.search);
    appid=urlParams.get('id');
    appid=urlParams.get('id');

    $('#button-configuration-view-hide').click( function(){
        //change button icon
        ...
        if($(this).find('span').hasClass('glyphicon-chevron-left')){
            $("#button-configuration-view-hide").parent().css('margin-right', '-300px'); 
        }else{
            $("#button-configuration-view-hide").parent().css('margin-right', '0px');
        }
     ...
     */
        ...
        dump:function()
        {
            ...
            writer.marshal(this.view, function (json) {
                ...
                // JOIN THE DIAGRAM MODEL WITH THE DATA MODEL 
                
                console.log(json);

                var j=0;
                var i=0;
                for(j; j<arrayObjectsModel.length; j++){
                    for(i=0; i<json.length; i++){                                        
                        if(arrayObjectsModel[j].id==json[i].id){
                            json[i].type=arrayObjectsModel[j].model;
                            json[i].name=arrayObjectsModel[j].name;
                            arrayObjectsModel[j].dataShape= json[i];    
                        }
                    }
                }
                ...
                for(i=0; i<json.length; i++){ 
                    if(json[i].type==null){
                        json[i].type="Connection"
                    }
                    if(json[i].type=="Connection"){
                        arrayObjectsModel.push(json[i])
                    }
                }
                //debugger
                return arrayObjectsModel;
            });
        },
        ...
        fileOpen: function(name)
        {
            ...
            var openByIdCallback = function(id){
                $.ajax({
                    ...
                    _this.view.clear();
                    var reader = new Reader();
                    reader.unmarshal(_this.view, loading);
            ...
            */
            ...
            // load dataShapes
            var modelDSLParsed = JSON.parse(modelDSL); // this is how you parse a string into JSON 
            console.log(modelDSLParsed);
            var dataShapes = []; 
            var j=0;
            var newArrayObjectsModel=[];
            for(j; j<modelDSLParsed.length; j++){
                if(modelDSLParsed[j].type=="Connection"){
                    dataShapes.push(modelDSLParsed[j])
                }
                else {
                    dataShapes.push(modelDSLParsed[j].dataShape) 
                    newArrayObjectsModel.push(modelDSLParsed[j])
                }
            }

            reader.unmarshal(this.view, dataShapes);
            arrayObjectsModel= newArrayObjectsModel;
        },
...
var EditEditPolicy = draw2d.policy.canvas.BoundingboxSelectionPolicy.extend({
    ...
    {
        // there is no benefit to show decorations during Drag&Drop of an shape --> sure?
        //
        ...
        if(hit!==null && hit.getParameterSettings().length>0){
            ...
            if(_this.configIcon===null) {
                _this.configIcon = $("<div class='ion-gear-a' style='opacity: 0.8;' id='configMenuIcon'></div>");
...
var ProbeWindow = Class.extend({
    ...
    show:function(force)
    {
        ...
        $("#probe_window").show().animate({height:'300px'},500);
...
var View = draw2d.Canvas.extend({
    ...
    init:function(app, id)
    {
        ...
        // path connection aqui attributes 
        ...
        var createConnection=function(sourcePort, targetPort){
            ...
            //alert(sourcePort + targetPort);
            // Alternatively you can register for this event with  
            c.on("dblclick", function(emitterFigure){
                alert("clicked");
            });
            return c;
        };
        ...
        $(".toolbar").delegate("#editDelete:not(.disabled)","click", function(){
            ...
            for (index = 0; index < selection.all.data.length; ++index) {

                var id = selection.all.data[index].id;
                //search and delete the model
                var index2=0;
                for (index2 = 0; index2 < arrayObjectsModel.length; ++index2) {
                    if(arrayObjectsModel[index2].id==id){
                        arrayObjectsModel.splice(index2, 1);
                        index2=0;
                    }
                }
                //search and delete references (connections)
                var index2=0;
                for (index2 = 0; index2 < arrayObjectsModel.length; ++index2) {
                   
                      for(r=0; r< arrayObjectsModel[index2].connections.length ; r++){
                           if(arrayObjectsModel[index2].connections[r].id==id){
                               //  debugger
                               arrayObjectsModel[index2].connections.splice(r, 1);
                              // console.log("estoy aads")
                              // debugger
                           }
                      }
    ...
    onDrop : function(droppedDomNode, x, y, shiftKey, ctrlKey)
    {
        ...
        var figure = eval("new "+type+"('" + name +"');"); // jshint ignore:line
    ...
    simulationStart:function()
    {
        ...
        // esto de aqui abajo era lo que venia
        if(this.simulate===true){
            return; // silently
        }
        ...
        //nuevo codigo de mario
        var div = document.getElementById("probe_hint");
        var html="<span class='ion-ios-information-outline'></span>Info:";

        // OPTION 1
        /*
        html += "<p></p><textarea readonly rows='3' cols='50' style='background: transparent;border: 0 none;width: 100%;font-size: 1em;resize: none;'>"
        html += "Loading system.... OK\n";        
        html += "Checking model..... ";
        // check model
        if(arrayObjectsModel.length==0){
            html += "ERROR: the model is empty.\n"
            //end
            html += "</textarea>";
            div.innerHTML = html;
            return;
        ...
        // Aquí añadir un iframe con la app que valida el modelo y demás....
        ...
        */
        ...
        //OPTION 2
        html += '<p></p><div id="iframe" style="display:block;"><iframe  src="/validator.html?model='+ encodeURIComponent(JSON.stringify(arrayObjectsModel)) +
            '&appid='+appid +'" width="100%" frameBorder="0"></iframe></div>';

        div.innerHTML = html;
        //create json with the DSL model and appid
        app.dump()
        var jsonDSL= {
            "appid": appid,
            "modelDSL": JSON.stringify(arrayObjectsModel)

        }
        //send to server 
        socket.emit('modelDSL', jsonDSL);
    ...
    return {
        ...
        { 
            ...
            var compiled = Handlebars.compile(
                ...
                '           <select class="form-control" id="figure_property_{{name}}" data-name="{{name}}" size="8"> '+
...
var MarkerFigure = draw2d.shape.layout.VerticalLayout.extend({
    ...
    init : function(attr, setter, getter)
    {
        ...
        // mario modificacion, tipos de puertos, en principio no lo necesito
        ...
        this.add(this.stateB = new MarkerStateBFigure({text:""}));
...
var hardware=(function(){
    ...
    return {
        ...
        init: function (s) {
            ...
            // LOAD DATA ***************************************************************
            socket.on('loadAPI', function(data){
                /*
                var jsonapi= JSON.parse(data);

                $("#paletteElements").append(
                    '  <div data-name="draw2d_circuit_'+ jsonapi.name+ '" class="mix col-md-6 pallette_item"> '+
                    '   <div class="glow"> ' +
                    ' <img data-shape="draw2d_circuit_Data" data-name="'+jsonapi.name +'" class="draw2d_droppable ui-draggable ui-draggable-handle"  ' + 
                    ' src="'+ jsonapi.imageUrl+'"> ' + 
                    ' <div><strong>'+ jsonapi.name+'</strong></div> </div> </div> ');
                var index;
                for (index = 0; index < jsonapi.params.length; ++index) {
                    jsonapi.params[index]["value"]="";
                }
                for (index = 0; index < jsonapi.values.length; ++index) {
                    jsonapi.values[index]["check"]="false";
                }

                jsonapi.id=globalId;
                globalId +=1;
                jsonapi.model="draw2d_circuit_Data";
                arrayShapesObjets[jsonapi.name]= jsonapi;
                ...
                */
                ...
                $("#paletteElements").append(
                    '  <div data-name="draw2d_circuit_'+ jsonapi.name+ '" class="mix col-md-6 pallette_item"> '+
                    '   <div class="glow"> ' +
                    ' <img data-shape="shape_DataSource" data-name="'+jsonapi.name +'" class="draw2d_droppable ui-draggable ui-draggable-handle"  ' + 
                    ' src="'+ jsonapi.imageUrl+'"> ' + 
                    ' <div><strong>'+ jsonapi.name+'</strong></div> </div> </div> ');
                var index;
                for (index = 0; index < jsonapi.params.length; ++index) {
                ...
                //jsonapi.id=globalId;
                //globalId +=1;
                jsonapi.model="shape_DataSource";
                jsonapi.connections=[];
                jsonapi.connectionsOut=[];
                arrayShapesObjets[jsonapi.name]= jsonapi;


            });
            socket.on('loadRequestAPI', function(data){

                numElemsApis=data;
                if(numElemsApis%2==0){
                    // add the "+" to the palette
                    $("#paletteElements").append(
                        '  <div data-name="_request_" class="mix col-md-12 pallette_item">'+
                        '  <a href="/addapi.html" target="_self">'+
                        '    <div class="request">'+
                        '       <div class="icon ion-ios-plus-outline"></div>'+
                        '       <div><strong>Add Data Sources</strong></div>'+
                        '   </div>'+
                        '   </a>  '+
                        '  </div>');
                }else{
            ...
            socket.on('loadDataPipeline', function(data){

                $("#paletteElements").append('<span class="title2">Data</span>');
                /*
                var newjson= {"name":"Pipeline2", "imageurl":"/resources/editor/assets/images/logos/monitor.png"};
                newjson.id= globalId;
                globalId +=1;
                newjson.type="PipelineSimple";
                newjson.model="draw2d_circuit_Pipeline";
                newjson.connections=[];
                arrayShapesObjets["Pipeline2"]= newjson;

                $("#paletteElements").append(
                    '  <div data-name="draw2d_circuit_pipeline" class="mix col-md-12 pallette_item"> '+
                    '  <div class="glow"> ' +
                    ' <img data-shape="draw2d_circuit_Pipeline" data-name="Pipeline" class="draw2d_droppable ui-draggable ui-draggable-handle"  ' + 
                    ' src="/resources/editor/assets/images/logos/monitor.png"> ' + 
                    ' <div><strong>Join Simple</strong></div> </div> </div> ');
                    */
                ...
                for(var i=0; i<array_shapes.length ; i++){
                    ...
                    if(array_shapes[i].getType()=="Results" && iniResults==0){
                        $("#paletteElements").append('<span class="title2">Dashboard</span>');
                        iniResults=1
                    }

                    //console.log(array_shapes[i].getType())
                    newjson = {"name": array_shapes[i].getName() , "imageUrl": array_shapes[i].getImageUrl()  }
                    //newjson.id= globalId;
                    //globalId +=1;
                    newjson.type=array_shapes[i].getType();
                    newjson.model=array_shapes[i].getModel();
                    newjson.connections=[];
                    newjson.connectionsOut=[];
                    if(newjson.type  == "Results" || newjson.type== "Storage")
                        newjson.url=array_shapes[i].getUrl()
                    arrayShapesObjets[array_shapes[i].getName()]= newjson;

                    $("#paletteElements").append(
                        '  <div data-name="'+ newjson.model  + '" class="mix col-md-12 pallette_item"> '+
                        '  <div class="glow"> ' +
                        ' <img data-shape="'+ newjson.model  + '" data-name="'+ newjson.name  + '" class="draw2d_droppable ui-draggable ui-draggable-handle"  ' + 
                        ' src="'+ newjson.imageUrl  + '"> ' + 
                        ' <div><strong>'+ newjson.name  + '</strong></div> </div> </div> ');
                ...
                //load DSL from modelDSL
                app.fileOpen();

            });


            //ask to server to load all info necessary
            socket.emit("loadInfoAPP" , ""); 
...
function destroyDiv(name) {
    if(name!=null)
        document.getElementById(name).remove();
}

function createLinkerConnection(sourcePort, targetPort){


    
    // check invalid schemas
    if(targetPort.name == "PortPipelineOut" && sourcePort.name == "PortDataOut"){
        return null;
    }
    if(targetPort.name == "PortResultsIn" && sourcePort.name == "PortDataOut"){
        return null;
    }
    if(targetPort.name == "PortProcessorIn" && sourcePort.name == "PortDataOut"){
        return null;
    }
    ...
    /*
    if(sourcePort.name == "PortPipelineOut" && targetPort.name == "PortResultsIn"){
        var label =  new draw2d.shape.basic.Label({text:"Filter"});
        label.fontSize += 5;
        label.bold=true;
        ...
        var locator= new draw2d.layout.locator.ManhattanMidpointLocator();
        label.setOnClickEvent("createHtmlPopUpFilter('" + sourcePort.parent.NAME.split('$')[1]  +"')");
        c.add(label, locator);
    }
    */

    //add connection to model
    if(targetPort.name == "PortPipelineIn"){
        //debugger;
        var model = getJsonModelFromArray(targetPort.parent.id, arrayObjectsModel);
        var connection= {id:sourcePort.parent.id};
        model.connections.push(connection);
        model = getJsonModelFromArray(sourcePort.parent.id, arrayObjectsModel);
        connection= {id:targetPort.parent.id};
        model.connections.push(connection);
    }
    else if(targetPort.name=="PortProcessorIn"){
        //save in
        var model = getJsonModelFromArray(targetPort.parent.id, arrayObjectsModel);
        var connection= {id:sourcePort.parent.id};
        model.connections.push(connection);
        //save out
        var model = getJsonModelFromArray(sourcePort.parent.id, arrayObjectsModel);
        debugger
        var connection= {id:targetPort.parent.id};
        model.connectionsOut.push(connection);
        
        
    }else if(targetPort.name=="PortResultsIn"){
        //save in
        var model = getJsonModelFromArray(targetPort.parent.id, arrayObjectsModel);
        var connection= {id:sourcePort.parent.id};
        model.connections.push(connection);
...
function getJsonModelFromArray(id, array){
    var index=0;
...
function getJsonModelFromArrayByName(name, array){
    var index=0;
    for (index = 0; index < array.length; ++index) {
